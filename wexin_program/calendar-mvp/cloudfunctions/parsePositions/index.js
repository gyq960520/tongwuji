...
function buildHsbcPrompt(_accountCurrency) {
  return `你是汇丰银行(中国) HSBC China 移动 App 截图识别助手。

【可能的页面类型 —— 先看顶部标题判断】

类型 A："我的账户" 页（账户总览）
  - 显示几行账户类别：
    - "活期存款账户 X,XXX,XXX.XX 人民币元"
    - "财富管理 X,XXX,XXX.XX 人民币元"（这是合计，详情在另一张 "我的持有" 截图里）
    - "信用卡"（跳过，不识别）
  - 处理：**只**提取"活期存款账户"作为 1 条 cash position（currency=CNY）
  - **不要**把"财富管理"作为 position 输出，它是合计，用户会另外上传 "我的持有" 截图提供明细
  - totalAssets = 活期存款账户 + 财富管理（手动相加这两个数）

类型 B："我的持有" 页（财富管理明细）
  - 顶部：持有总额 X,XXX,XXX.XX 人民币 → totalAssets
  - 下方"基金相关产品 (N) X,XXX,XXX.XX 人民币"是该类合计，**跳过不输出**
  - 再下方 N 个产品卡片，每个含：
    - 产品全名（带 "汇丰代客境外理财计划 -" 前缀和 "- 分派" 后缀的长名）
    - 灰色 tag "代客境外理财-海外基金（自有）"（跳过这个 tag）
    - "参考市值 X,XXX.XX 人民币" 或 "X,XXX.XX 美元"
    - "未实现损益(率)"（跳过）
    - "现金分红 X,XXX.XX 人民币"（跳过 —— 用户已提示现金分红已进活期，不需重复记）
    - "总收益(率)"（跳过）
  - 处理：**每个产品输出 1 条 wealth position**
  - totalAssets = 持有总额

【name 缩写规则（重要）】
HSBC 产品名通常是 "汇丰代客境外理财计划 - 富达美元债券基金 - 人民币对冲 - 分派" 这种长格式。
- **去掉前缀** "汇丰代客境外理财计划 -" / "汇丰代客境外理财计划-"
- **去掉后缀** " - 分派" / " - 累积" 等
- 中间的对冲信息 "- 人民币对冲 -" 或 "- 美元 -" 也去掉
- 保留**核心基金名**
- 例：
...
【category】
- 活期存款账户 → "cash"
- 财富管理下的所有产品（境外基金 / 债券基金 / 各类理财） → "wealth"
- 信用卡 → 跳过

【其他规则】
1. 信用卡 / 信用卡额度 / 信用卡欠款 一律不识别
2. 现金分红 / 未实现损益 / 总收益 / 票息 都跳过，只取"参考市值"作为 amount
3. 数字里的逗号去掉
4. 金额为 0 不输出
5. shares 和 price 全部留 null（HSBC 理财产品不显示份额/单价）
6. 只输出纯 JSON 对象
...
【输出结构】
{
  "totalAssets": 数字 | null,
  "positions": [
    {"name":"...","code":"","category":"cash|wealth","currency":"CNY|USD|HKD","shares":null,"price":null,"amount":数字}
  ]
}

【示例 A：处理 "我的账户" 页（活期 368,297.23 + 财富管理 136,708.17）】
{
...
【示例 B：处理 "我的持有" 页（3 个境外基金，持有总额 136,708.17）】
{
  "totalAssets": 136708.17,
  "positions": [
    {"name":"富达美元债券基金","code":"","category":"wealth","currency":"CNY","shares":null,"price":null,"amount":44624.55},
    {"name":"摩根环球企业债券基金","code":"","category":"wealth","currency":"CNY","shares":null,"price":null,"amount":46893.55},
    {"name":"汇丰气候转型环球企业债券基金","code":"","category":"wealth","currency":"USD","shares":null,"price":null,"amount":6649.61}
  ]
}
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    const content = apiRes && apiRes.choices && apiRes.choices[0] && apiRes.choices[0].message
      ? apiRes.choices[0].message.content || ''
      : ''
    console.log('[parsePositions] 模型原文 (前 500 字):', content.slice(0, 500))

...
3. **"资金"/"可用资金" 必须屏幕上看到"资金"或"可用"这两个字本身才识别** → 一条 position，name 严格写"资金"或"可用资金"。**不要凭"位置在表格顶部 + 只有一行数字"推断成资金** —— 用户多张滚动截图衔接处常出现被切断的产品残片：表头（"证券/市值 数量/可用 现价/成本 ..."）下方第一行没有证券名、只见一坨数字（例 "39550.00 / 200 / 274.2473 / -27.89% / 0.00%"），这是产品上半截在另一张截图、下半截在当前截图。**这种残片整行跳过，不识别为任何 position（既不是产品也不是资金）**。识别口诀：有"资金/可用"字 = 资金；有证券名 = 产品；只有孤立数字 = 截图残片跳过。
4. **区分两个不同概念**：
